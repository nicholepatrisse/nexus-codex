...
import { listCatalogMaterials, listOwnedMaterials } from "@/materials/materials";
...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<{ tab?: string | string[] }> }) {
  ...
  return <main className="page-shell mx-auto min-h-screen max-w-3xl"><section className="card-standard responsive-card sm:rounded-3xl sm:p-10">
    ...
    {tab === "notifications" && notificationPreferences ? <NotificationPreferencesForm preferences={notificationPreferences} /> : null}
    {tab === "materials" && materials && catalog ? <div className="mt-8"><MaterialsOwned materials={materials} catalog={catalog} removeAction={removeMaterialAction} /></div> : null}
  </section></main>;
}
