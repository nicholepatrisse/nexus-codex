...
export function ApplicationHeader({ notifications, notificationsError = false }: { notifications: AppNotification[]; notificationsError?: boolean }) {
  ...
  const storageKey = session ? `nexus-notifications-seen:${session.user.id}` : null;
  const badgeCount = useMemo(() => notificationBadgeCount(notifications, seen), [notifications, seen]);
  function toggleNotifications() {
    const nextOpen = !open; setOpen(nextOpen);
    if (!nextOpen || !storageKey) return;
    let stored: string[] = [];
    try { stored = JSON.parse(localStorage.getItem(storageKey) ?? "[]") as string[]; } catch { stored = []; }
    const nextSeen = new Set([...stored, ...seen, ...notifications.map(({ id }) => id)]); setSeen(nextSeen); localStorage.setItem(storageKey, JSON.stringify([...nextSeen]));
  }
