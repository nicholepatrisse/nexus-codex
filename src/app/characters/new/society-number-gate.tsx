...
export function SocietyNumberGate({ initialSocietyPlayNumber, usedCharacterNumbers, validationContext, returnTo }: { initialSocietyPlayNumber: string; usedCharacterNumbers: string[]; validationContext: IdentityValidationContext; returnTo?: string }) {
  ...
  return <><CharacterForm societyPlayNumber={societyPlayNumber} usedCharacterNumbers={usedCharacterNumbers} validationContext={validationContext} returnTo={returnTo} />{!societyPlayNumber ? <Dialog open title="Add your society number" description="Your society number is needed to create organized-play characters." eyebrow="Profile required" onClose={() => {}} closePolicy="none" showCloseButton={false} className="max-w-md">
