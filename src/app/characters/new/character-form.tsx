...
export function CharacterForm({ societyPlayNumber, returnTo }: { societyPlayNumber: string; returnTo?: string }) {
  ...
  return <form action={action} className="mt-10 space-y-7" noValidate>
    ...
    <fieldset><legend className="block text-sm font-semibold">Society identification</legend><div className="mt-2 grid grid-cols-[1fr_auto_1fr] items-start gap-3">
      ...
      <div><label htmlFor="characterNumber" className="sr-only">Character sequence number</label><div className="flex rounded-xl border border-white/15 bg-white/5 focus-within:border-[var(--accent)]"><span aria-hidden="true" className="border-r border-white/15 px-4 py-3 text-[var(--muted)]">27</span><input id="characterNumber" name="characterNumber" required inputMode="numeric" maxLength={2} pattern="(?:0?[1-9]|[1-9][0-9])" placeholder="01" aria-label="Character sequence number, 1 to 99" aria-invalid={Boolean(field("characterNumber"))} onBlur={(event) => { event.currentTarget.value = normalizeCharacterNumber(event.currentTarget.value); }} className="min-w-0 flex-1 bg-transparent px-4 py-3 outline-none" /></div>{field("characterNumber") ? <p role="alert" className="mt-2 text-sm text-red-300">{field("characterNumber")}</p> : null}</div>
    ...
    <div className="flex items-center gap-4"><SubmitButton /><Link href={returnTo ?? "/characters"} className="text-sm text-[var(--muted)] hover:text-white">Cancel</Link></div>
  </form>;
}
