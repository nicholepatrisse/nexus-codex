...
export function CharacterForm({ societyPlayNumber, usedCharacterNumbers, returnTo }: { societyPlayNumber: string; usedCharacterNumbers: string[]; returnTo?: string }) {
  ...
  return <form action={action} className="relative mt-10 space-y-7" noValidate>
    ...
    <fieldset><legend className="block text-sm font-semibold">Character details</legend><p className="mt-1 text-sm text-text-muted">Choose the permanent Society starting level. You can fill in or change the other details later.</p><div className="mt-4 grid gap-5 sm:grid-cols-2">
      ...
      <fieldset className="sm:col-span-2"><legend className="text-sm font-semibold">Starting wealth</legend><p className="mt-1 text-sm text-text-muted">Choose one option for your level {startingLevel} character.</p><div className={`mt-3 grid gap-3 ${SFS2_STARTING_WEALTH[startingLevel].length > 1 ? "sm:grid-cols-2" : ""}`}>{SFS2_STARTING_WEALTH[startingLevel].map((option) => <label key={option.credits} className="flex cursor-pointer items-start gap-3 rounded-xl border border-border-strong bg-surface-raised p-4 transition-colors hover:bg-surface-hover has-checked:border-brand has-checked:ring-1 has-checked:ring-brand"><input type="radio" name="startingCredits" value={option.credits} required checked={startingCredits === option.credits} onChange={() => { setStartingCredits(option.credits); setStartingItems([]); }} className="mt-1 size-4 shrink-0 appearance-none rounded-full border-2 border-border-strong bg-surface transition checked:border-4 checked:border-brand checked:bg-on-brand focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand" /><span className="min-w-0"><span className="block text-sm font-semibold">{option.kind === "credits_only" ? "Credits only" : "Permanent items"}</span><span className="mt-1 block text-sm leading-6 text-text-muted">{option.label}</span></span></label>)}</div>{field("startingCredits") ? <p role="alert" className="mt-2 text-sm text-danger">{field("startingCredits")}</p> : null}</fieldset>
