...
import { identitySelectionNeedsChronicle, type IdentityValidationContext, type IdentityValidationOption } from "@/character/identity-validation";
import { IdentityOptionField } from "@/character/identity-option-field";
import type { CharacterDetail } from "@/character/characters";
import type { listOwnedChronicles } from "@/character/chronicles";
...
export function EditCharacterForm({ character, validationContext, chronicles, characterOptions }: { character: CharacterDetail; validationContext: IdentityValidationContext; chronicles: Awaited<ReturnType<typeof listOwnedChronicles>>; characterOptions: CharacterOptionDraft[] }) {
  ...
  const [ancestrySourceChronicleId, setAncestrySourceChronicleId] = useState(character.ancestrySourceChronicleId ?? "");
  const [background, setBackground] = useState(character.background ?? "");
  const [backgroundSourceChronicleId, setBackgroundSourceChronicleId] = useState(character.backgroundSourceChronicleId ?? "");
  const [importedOptions, setImportedOptions] = useState<IdentityValidationOption[]>([]);
  const [addedMaterialIdentities, setAddedMaterialIdentities] = useState<string[]>([]);
  const currentValidationContext = { options: [...validationContext.options, ...importedOptions], ownedMaterialIdentities: [...validationContext.ownedMaterialIdentities, ...addedMaterialIdentities] };
  const recordImportedOption = (option: IdentityValidationOption) => setImportedOptions((current) => [...current, option]);
  const recordAddedMaterials = (identities: string[]) => setAddedMaterialIdentities((current) => [...new Set([...current, ...identities])]);
  const [validationNotes, setValidationNotes] = useState({ ancestry: character.ancestryValidationNote ?? "", background: character.backgroundValidationNote ?? "" });
  ...
  const [optionDrafts, setOptionDrafts] = useState(characterOptions);
  ...
  return <form action={formAction} className="relative mt-10 space-y-6" noValidate>
    ...
    <PathmuncherImport characterId={character.id} onApply={(values) => { setName(values.name); setClassName(values.className); setAncestry(values.ancestry); setBackground(values.background); setOptionDrafts(values.options); setImportRevision((value) => value + 1); }} />
    <div><label htmlFor="name" className="block text-sm font-semibold">Character name</label><input id="name" name="name" required maxLength={100} value={name} onChange={(event) => setName(event.currentTarget.value)} aria-invalid={Boolean(field("name"))} className={`mt-2 ${inputClass}`} />{field("name") ? <p role="alert" className="mt-2 text-sm text-danger">{field("name")}</p> : null}</div>
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <div><input type="hidden" name="classValidationNote" value={character.classValidationNote ?? ""} /><label htmlFor="className" className="block text-sm font-semibold">Class <span className="font-normal text-text-muted">(optional)</span></label><CharacterClassSelect key={`${importRevision}-${className}`} defaultValue={className} allowUnknown invalid={Boolean(field("className"))} onValueChange={setClassName} />{field("className") ? <p role="alert" className="mt-2 text-sm text-danger">{field("className")}</p> : null}</div>
      <div className="sm:col-span-2"><IdentityOptionField type="background" value={background} onValueChange={setBackground} note={validationNotes.background} onNoteChange={(value) => setValidationNote("background", value)} context={currentValidationContext} hasChronicleAccess={Boolean(backgroundSourceChronicleId)} invalid={Boolean(field("background"))} onOptionImported={recordImportedOption} onMaterialAdded={recordAddedMaterials} />{identitySelectionNeedsChronicle("background", background, currentValidationContext) ? <div className="mt-3"><label htmlFor="backgroundSourceChronicleId" className="block text-sm font-semibold">Source Chronicle <span className="font-normal text-text-muted">(optional)</span></label><StyledSelect name="backgroundSourceChronicleId" label="Background Source Chronicle" value={backgroundSourceChronicleId} onValueChange={setBackgroundSourceChronicleId} invalid={Boolean(field("backgroundSourceChronicleId"))} options={[{ value: "", label: "No Chronicle" }, ...chronicles.map((chronicle) => ({ value: chronicle.id, label: `${chronicle.characterName}: ${chronicle.scenarioNumberSnapshot} — ${chronicle.scenarioNameSnapshot}` }))]} />{field("backgroundSourceChronicleId") ? <p role="alert" className="mt-2 text-sm text-danger">{field("backgroundSourceChronicleId")}</p> : null}</div> : <input type="hidden" name="backgroundSourceChronicleId" value={backgroundSourceChronicleId} />}{field("background") ? <p role="alert" className="mt-2 text-sm text-danger">{field("background")}</p> : null}</div>
      <div className="sm:col-span-2"><IdentityOptionField type="ancestry" value={ancestry} onValueChange={setAncestry} note={validationNotes.ancestry} onNoteChange={(value) => setValidationNote("ancestry", value)} context={currentValidationContext} hasChronicleAccess={Boolean(ancestrySourceChronicleId)} invalid={Boolean(field("ancestry"))} onOptionImported={recordImportedOption} onMaterialAdded={recordAddedMaterials} />{identitySelectionNeedsChronicle("ancestry", ancestry, currentValidationContext) ? <div className="mt-3"><label htmlFor="ancestrySourceChronicleId" className="block text-sm font-semibold">Source Chronicle <span className="font-normal text-text-muted">(optional)</span></label><StyledSelect name="ancestrySourceChronicleId" label="Ancestry Source Chronicle" value={ancestrySourceChronicleId} onValueChange={setAncestrySourceChronicleId} invalid={Boolean(field("ancestrySourceChronicleId"))} options={[{ value: "", label: "No Chronicle" }, ...chronicles.map((chronicle) => ({ value: chronicle.id, label: `${chronicle.characterName}: ${chronicle.scenarioNumberSnapshot} — ${chronicle.scenarioNameSnapshot}` }))]} />{field("ancestrySourceChronicleId") ? <p role="alert" className="mt-2 text-sm text-danger">{field("ancestrySourceChronicleId")}</p> : null}</div> : <input type="hidden" name="ancestrySourceChronicleId" value={ancestrySourceChronicleId} />}{field("ancestry") ? <p role="alert" className="mt-2 text-sm text-danger">{field("ancestry")}</p> : null}</div>
      ...
      <div id="heritage-feats" className="sm:col-span-2"><CharacterOptionFields key={importRevision} initial={optionDrafts} startingLevel={character.currentLevel} /></div>
