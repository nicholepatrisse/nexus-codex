...
import { focusFirstCharacterFormError } from "@/app/characters/character-form-section";
...
export function EditCharacterForm({ character, validationContext, chronicles }: { character: CharacterDetail; validationContext: IdentityValidationContext; chronicles: Awaited<ReturnType<typeof listOwnedChronicles>> }) {
  ...
  useEffect(() => { if (formRef.current && state.fieldErrors) focusFirstCharacterFormError(formRef.current, state.fieldErrors); }, [state.fieldErrors]);
  return <form ref={formRef} action={formAction} className="relative mt-10 space-y-6" noValidate>
