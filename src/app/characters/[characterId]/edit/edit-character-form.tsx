...
export function EditCharacterForm({ character, validationContext }: { character: CharacterDetail; validationContext: IdentityValidationContext }) {
  ...
  return <form action={formAction} className="relative mt-10 space-y-6" noValidate>
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      {(["background", "ancestry"] as const).map((type) => <div className="sm:col-span-2" key={type}><IdentityOptionField type={type} value={type === "ancestry" ? ancestry : background} onValueChange={type === "ancestry" ? setAncestry : setBackground} note={validationNotes[type]} onNoteChange={(value) => setValidationNote(type, value)} context={validationContext} invalid={Boolean(field(type))} />{field(type) ? <p role="alert" className="mt-2 text-sm text-danger">{field(type)}</p> : null}</div>)}
