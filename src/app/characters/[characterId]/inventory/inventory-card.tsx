...
export function InventoryCard({ characterId, entry, chronicles, sourceChronicle, chronicleAction, saleAction, returnAction }: { characterId: string; entry: ValidatedInventoryEntry; chronicles: ChronicleWithGmCredit[]; sourceChronicle?: ChronicleWithGmCredit; chronicleAction: (data: FormData) => Promise<{ ok: boolean; error?: string }>; saleAction: (state: SaleState, data: FormData) => Promise<SaleState>; returnAction: (state: SaleState, data: FormData) => Promise<SaleState> }) {
  ...
  return <li className="card-standard card-interactive group relative overflow-hidden">
    ...
    <SaleForm key={entry.quantity} itemName={entry.itemNameSnapshot} available={entry.quantity} valueMinor={entry.valueMinor} returnAmount={entry.acquisitionType === "purchased" && entry.sourcePurchaseId ? entry.amountPaidMinor : null} editHref={`/characters/${characterId}/inventory/${entry.id}/edit`} action={saleAction} returnAction={returnAction} triggerClassName="pointer-events-none absolute top-1/2 right-3 z-10 -translate-y-1/2 opacity-0 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100" />
