...
export default async function NewInventoryPage({ params }: { params: Promise<{ characterId: string }> }) { const { characterId } = await params; const actor = await getAuthenticatedActor(); if (!actor) redirect(`/sign-in?returnTo=/characters/${characterId}/inventory/new`); const character = await getCharacterDetail(actor, characterId); if (!character?.isOwner) notFound(); const [chronicles, ownedMaterialIdentities] = await Promise.all([listChronicles(characterId), listOwnedMaterialIdentities(actor)]); return <main className="mx-auto min-h-screen max-w-2xl px-6 py-16"><Link className="text-sm text-brand hover:underline" href={`/characters/${characterId}?tab=inventory`}>← {character.name} Inventory</Link><h1 className="mt-8 text-4xl font-semibold">Add item</h1><p className="mt-2 text-text-muted">Choose how the item was acquired. Purchased items debit credits; other items only update inventory.</p><InventoryForm characterId={characterId} chronicles={chronicles} ownedMaterialIdentities={ownedMaterialIdentities} idempotencyKey={randomUUID()} action={createInventoryAction.bind(null, characterId)} /></main>; }
