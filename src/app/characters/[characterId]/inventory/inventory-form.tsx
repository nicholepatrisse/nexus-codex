...
export function InventoryForm({ characterId, entry, chronicles, action }: { characterId: string; entry?: InventoryEntry; chronicles: ChronicleOption[]; action: (state: InventoryFormState, data: FormData) => Promise<InventoryFormState> }) {
  ...
  return <form action={formAction} className="mt-8 space-y-5">
    ...
    <div className="grid gap-5 sm:grid-cols-2"><div><label className="text-sm font-semibold" htmlFor="acquiredOn">Acquired on</label><input className={field} id="acquiredOn" name="acquiredOn" type="date" required defaultValue={entry?.acquiredOn ?? new Date().toISOString().slice(0,10)} /><ErrorText errors={state.fieldErrors?.acquiredOn} /></div><div><label className="text-sm font-semibold" htmlFor="amountPaidMinor">Exact amount paid <span className="font-normal text-text-muted">(optional credits)</span></label><input className={field} id="amountPaidMinor" name="amountPaidMinor" type="number" min="0" step="1" defaultValue={entry?.amountPaidMinor ?? ""} /><ErrorText errors={state.fieldErrors?.amountPaidMinor} /></div></div>
