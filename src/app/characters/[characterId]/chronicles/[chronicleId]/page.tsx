...
import { applyChronicleAction, unapplyChronicleAction } from "../actions";
...
export default async function ChronicleDetailPage({ params }: { params: Promise<{ characterId: string; chronicleId: string }> }) {
  ...
  return <main className="mx-auto min-h-screen max-w-3xl px-6 py-16">
    ...
    <article className="mt-8 rounded-3xl border border-border bg-surface p-8 sm:p-10">
      ...
      {character.isOwner && chronicle.status === "pending" && chronicle.creditType === "pregen" && chronicle.characterLevel > 1 && character.currentLevel === 1 && chronicle.eligibilityState === "eligible" ? <div className="mt-8 flex items-center justify-between gap-4 border-t border-border pt-8"><div><h2 className="font-semibold">Level-1 pregen credit</h2><p className="mt-1 text-sm text-text-muted">Apply this Chronicle now, or leave it held until this character reaches level {chronicle.characterLevel}.</p></div><ChronicleLifecycleButton status="pending" action={applyChronicleAction.bind(null, characterId, chronicleId)} /></div> : null}
    </article>
  </main>;
}
