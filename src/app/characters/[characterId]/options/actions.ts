...
import { characterOptionSelectionInputSchema, deleteCharacterOptionSelection, updateCharacterOptionSelection } from "@/character/option-selections";
...
export async function updateOptionCardAction(characterId: string, selectionId: string, _state: OptionCardState, formData: FormData): Promise<OptionCardState> {
  const parsed = characterOptionSelectionInputSchema.safeParse({
    selectionKind: formData.get("selectionKind"),
    name: formData.get("name"),
    acquiredLevel: formData.get("acquiredLevel"),
    featCategory: formData.get("featCategory") || null,
    acquisitionMethod: formData.get("acquisitionMethod") || null,
    grantOrigin: formData.get("grantOrigin") || null,
    validationNote: formData.get("validationNote") || null,
    characterOptionId: formData.get("characterOptionId") || null,
    sourceMaterialIdentity: formData.get("sourceMaterialIdentity") || null,
    sourceMaterialTitle: formData.get("sourceMaterialTitle") || null,
    sourceUrl: formData.get("sourceUrl") || null,
    sourceChronicleId: formData.get("sourceChronicleId") || null,
  });
