    ...
    <section className="relative mt-8 rounded-3xl border border-border bg-surface p-8 sm:p-10">
      ...
      <section className="mt-10 border-t border-border pt-8">
        ...
        {!chronicles.length ? <p className="mt-4 text-sm text-text-muted">No Chronicles recorded yet.</p> : <ul className="mt-5 space-y-4">{chronicles.map((chronicle) => <li key={chronicle.id} className="rounded-xl border border-border bg-surface-raised p-5">
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4"><div><dt className="text-text-muted">XP</dt><dd className="font-semibold">{chronicle.xp}</dd></div>{character.isOwner ? <><div><dt className="text-text-muted">Base credits</dt><dd className="font-semibold">{formatCredits(chronicle.baseCreditsMinor)}</dd></div><div><dt className="text-text-muted">Downtime credits</dt><dd className="font-semibold">{formatCredits(chronicle.downtimeCreditsMinor)}</dd></div><div><dt className="text-text-muted">Total credits</dt><dd className="font-semibold">{formatCredits(totalCredits(chronicle))}</dd></div></> : null}<div><dt className="text-text-muted">Downtime</dt><dd className="font-semibold">{chronicle.downtimeDays} days · {chronicle.downtimeDisposition.replaceAll("_", " ")}</dd></div></dl>
          {chronicle.partnerCode || chronicle.chronicleNumber || chronicle.eventName || chronicle.eventCode || chronicle.gmOrganizedPlayId ? <p className="mt-3 text-sm text-text-muted">{[chronicle.chronicleNumber && `Chronicle ${chronicle.chronicleNumber}`, chronicle.partnerCode && `Partner ${chronicle.partnerCode}`, chronicle.eventName, chronicle.eventCode, chronicle.gmOrganizedPlayId && `GM ${chronicle.gmOrganizedPlayId}`].filter(Boolean).join(" · ")}</p> : null}
          ...
          {character.isOwner && (chronicle.provenance === "manual" || chronicle.status === "pending") ? <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-border pt-3">{chronicle.provenance === "manual" ? <><Link href={`/characters/${character.id}/chronicles/${chronicle.id}/edit`} className="rounded-full border border-border-strong px-4 py-2 text-sm font-semibold text-text-primary transition-colors hover:border-brand hover:text-brand focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand">Edit Chronicle</Link>{chronicle.status === "pending" ? <DeleteChronicleButton scenario={`${chronicle.scenarioNumberSnapshot} — ${chronicle.scenarioNameSnapshot}`} action={deleteChronicleAction.bind(null, character.id, chronicle.id)} /> : null}</> : <p className="text-sm text-text-muted">Review this GM-issued Chronicle, then apply it to add its rewards.</p>}<div className="ml-auto"><ChronicleLifecycleButton status={chronicle.status as "pending" | "applied"} action={(chronicle.status === "pending" ? applyChronicleAction : unapplyChronicleAction).bind(null, character.id, chronicle.id)} /></div></div> : null}
        </li>)}</ul>}
      </section>
