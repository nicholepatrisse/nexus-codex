...
export default async function CharacterPage({ params, searchParams }: { params: Promise<{ characterId: string }>; searchParams: Promise<{ tab?: string | string[] }> }) {
  ...
  return <main className="page-shell mx-auto min-h-screen max-w-3xl">
    <section className="card-standard responsive-card relative sm:rounded-3xl sm:p-10">
      ...
      {characterOptions?.length ? <section className="mt-8 border-t border-border pt-8"><h2 className="text-xl font-semibold">Heritage and feats</h2>{characterOptions.filter(({ selectionKind }) => selectionKind === "heritage").map((heritage) => <div key={heritage.id} className="mt-4"><h3 className="text-sm font-semibold text-text-muted">Heritage</h3><p className="mt-1 font-semibold">{heritage.nameSnapshot}</p></div>)}<div className="mt-6 grid gap-6 sm:grid-cols-2">{(["class", "ancestry", "skill", "general", null] as const).map((category) => { const feats = characterOptions.filter((option) => option.selectionKind === "feat" && option.featCategory === category); if (!feats.length) return null; return <section key={category ?? "unknown"}><h3 className="font-semibold capitalize">{category ? `${category} feats` : "Other feats"}</h3><ul className="mt-2 space-y-2">{feats.map((feat) => <li key={feat.id} className="rounded-xl border border-border bg-surface-raised p-3"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold">{feat.nameSnapshot}</span>{feat.acquisitionMethod === "awarded" ? <span className="rounded-full bg-brand/10 px-2 py-0.5 text-xs font-semibold text-brand">Awarded</span> : null}</div><p className="mt-1 text-sm text-text-muted">Acquired at level {feat.acquiredLevel}{feat.grantOrigin ? ` · ${feat.grantOrigin}` : ""}</p></li>)}</ul></section>; })}</div></section> : null}
