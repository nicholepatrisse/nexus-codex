...
export default async function CharactersPage() {
  ...
  return <main className="mx-auto min-h-screen max-w-3xl px-6 py-16">
    ...
    {characterList.length ? <ul className="mt-10 space-y-4">{characterList.map((character) => <li key={character.id} className="rounded-2xl border border-border bg-surface p-6"><h2 className="text-xl font-semibold"><Link className="hover:text-brand hover:underline" href={`/characters/${character.id}`}>{character.name}</Link></h2><p className="mt-2 text-text-muted">{character.societyNumber} · Level {character.currentLevel} · {character.totalXp} XP</p></li>)}</ul> : <section className="mt-10 rounded-2xl border border-dashed border-border-strong p-8 text-center"><h2 className="text-xl font-semibold">No characters yet</h2><p className="mt-2 text-text-muted">Add your first character to use in games.</p></section>}
  </main>;
}
