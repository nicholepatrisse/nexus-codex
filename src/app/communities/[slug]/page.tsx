...
export default async function CommunityPage({ params, searchParams }: CommunityPageProps) {
  ...
  const publishedSessions = canViewSchedule
    ? await getDb().select({ id: sessions.id, code: contentItems.code, title: contentItems.title, startsAt: sessions.startsAt, gmName: people.displayName, gmPersonId: sessions.gmPersonId, gameSystemId: rulesets.gameSystemId, status: sessions.status, paizoReportedAt: sessions.paizoReportedAt }).from(sessions).innerJoin(contentItems, eq(contentItems.id, sessions.contentItemId)).innerJoin(organizedPlayPrograms, eq(organizedPlayPrograms.id, contentItems.programId)).innerJoin(rulesets, eq(rulesets.id, organizedPlayPrograms.rulesetId)).innerJoin(people, eq(people.id, sessions.gmPersonId)).where(and(eq(sessions.communityId, community.id), inArray(sessions.status, ["published", "completed"]))).orderBy(asc(sessions.startsAt))
    : [];
  const ownSignups = actor && publishedSessions.length
    ? await getDb().select({ sessionId: sessionSignups.sessionId, status: sessionSignups.status, characterId: sessionSignups.characterId, characterName: characters.name, pregenName: sessionSignups.pregenName }).from(sessionSignups).leftJoin(characters, eq(characters.id, sessionSignups.characterId)).where(and(
