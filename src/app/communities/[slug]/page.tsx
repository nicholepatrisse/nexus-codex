...
export default async function CommunityPage({ params, searchParams }: CommunityPageProps) {
  ...
  const ownSignups = actor && publishedSessions.length
    ? await getDb().select({ sessionId: sessionSignups.sessionId, status: sessionSignups.status, characterName: characters.name }).from(sessionSignups).leftJoin(characters, eq(characters.id, sessionSignups.characterId)).where(and(
  ...
  return <CommunityProfile community={community} isOwner={isOwner} isSignedIn={Boolean(actor)} isMember={authorization.access.isActiveMember} pendingRequestId={pendingRequest?.id} gmAdmission={community.gmAdmission === "self_service" ? "self_service" : "approved_only"} gmState={gmState} pendingGmRequestId={gmRequest?.status === "pending" ? gmRequest.id : undefined} drafts={drafts.map((draft) => ({ ...draft, startsAt: draft.startsAt.toISOString() }))} sessions={publishedSessions.map((session) => { const signup = signupBySession.get(session.id); return { ...session, startsAt: session.startsAt.toISOString(), canSignUp: actor?.personId !== session.gmPersonId, signupStatus: signup?.status === "confirmed" ? "confirmed" as const : signup?.status === "waitlisted" ? "waitlisted" as const : undefined, signupCharacterName: signup?.characterName ?? undefined, eligibleCharacters: ownCharacters.filter(({ gameSystemId }) => gameSystemId === session.gameSystemId).map(({ id, name, societyNumber }) => ({ id, name, societyNumber })) }; })} canViewSchedule={canViewSchedule} published={(await searchParams)?.published === "1"} />;
}
