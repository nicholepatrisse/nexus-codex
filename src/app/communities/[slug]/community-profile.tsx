...
export type CommunityProfileProps = Readonly<{
  ...
  sessions?: { id: string; code: string; title: string; startsAt: string; gmName: string }[];
  published?: boolean;
}>;
...
export function CommunityProfile({ community, isOwner, isSignedIn = false, isMember = false, pendingRequestId, gmAdmission = "approved_only", gmState, pendingGmRequestId, drafts = [], sessions = [], published = false }: CommunityProfileProps) {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-4xl px-6 py-16 sm:py-24">
      ...
      {sessions.length ? <section className="mt-8 rounded-3xl border border-white/10 bg-black/20 p-8"><h2 className="text-2xl font-semibold">Published sessions</h2><ul className="mt-5 space-y-3">{sessions.map((session) => <li key={session.id} className="rounded-xl border border-white/10 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">{session.code} — {session.title}</p><p className="mt-1 text-sm text-[var(--muted)]">{new Date(session.startsAt).toLocaleString()}</p><p className="mt-1 text-sm text-[var(--muted)]">GM: {session.gmName}</p></div><span className="rounded-full bg-emerald-300/10 px-3 py-1 text-xs font-semibold text-emerald-100">Published</span></div></li>)}</ul></section> : null}
