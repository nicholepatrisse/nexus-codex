...
export function SessionSignupControl({
  ...
  return <div className="mt-4">
    {status ? <div><div className="flex flex-wrap items-center gap-3"><p className="text-sm text-success">{status === "confirmed" ? "You’re confirmed" : `You’re waitlisted${signupState.waitlistPosition ? ` at position ${signupState.waitlistPosition}` : ""}`}{initialCharacterName ? ` as ${initialCharacterName}` : ""}.</p><button type="button" onClick={() => setEditing((value) => !value)} className="text-sm font-semibold text-brand hover:underline">Edit signup</button><form action={cancelAction} onSubmit={(event) => { if (!window.confirm("Cancel your signup? This will remove you from the game/session.")) event.preventDefault(); }}><button type="submit" disabled={cancelPending} className="text-sm font-semibold text-danger hover:underline disabled:opacity-60">{cancelPending ? "Cancelling…" : "Cancel signup"}</button></form></div>{editing ? <form action={updateAction} className="mt-4"><fieldset><legend className="text-sm font-semibold">Character</legend><div className="mt-2 grid gap-3 sm:grid-cols-2">{characters.map((character) => <SelectionCard key={character.id} name="characterId" value={character.id} required title={character.name} description={character.societyNumber} metadata={`Level ${character.currentLevel}`} checked={characterId === character.id} onChange={() => setCharacterId(character.id)} />)}</div></fieldset><div className="mt-3 flex items-center gap-3"><button type="submit" disabled={updatePending} className="rounded-full bg-brand px-4 py-2 text-sm font-semibold text-on-brand disabled:opacity-60">{updatePending ? "Saving…" : "Save signup"}</button>{updateState.status === "updated" ? <p role="status" className="text-sm text-success">Signup updated.</p> : null}</div></form> : null}</div> : characters.length ? <form action={signupAction}><fieldset><legend className="text-sm font-semibold">Choose a character</legend><div className="mt-2 grid gap-3 sm:grid-cols-2">{characters.map((character) => <SelectionCard key={character.id} name="characterId" value={character.id} required title={character.name} description={character.societyNumber} metadata={`Level ${character.currentLevel}`} checked={characterId === character.id} onChange={() => setCharacterId(character.id)} />)}</div></fieldset><button type="submit" disabled={signupPending || !characterId} className="mt-3 rounded-full bg-brand px-4 py-2 text-sm font-semibold text-on-brand disabled:opacity-60">{signupPending ? "Signing up…" : "Sign up"}</button></form> : <p className="text-sm text-text-muted">You need an eligible character before you can sign up. <Link href={`/characters/new?returnTo=${encodeURIComponent(returnTo)}`} className="font-semibold text-brand hover:underline">Add a character</Link></p>}
