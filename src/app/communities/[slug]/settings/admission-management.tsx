...
type Invitation = { id: string; status: string; maxUses: number | null; useCount: number; expiresAt: Date };
...
export function AdmissionManagement({ slug, invitations, requests }: { slug: string; invitations: Invitation[]; requests: Request[] }) {
  return <section className="mt-8 rounded-3xl border border-white/10 bg-black/20 p-8"><h2 className="text-2xl font-semibold">Sharing links and membership</h2><p className="mt-3 text-sm text-[var(--muted)]">Generate a link and share it wherever you coordinate your community. Email delivery can be added later.</p><div className="mt-6"><CreateInvitation slug={slug} /></div><h3 className="mt-8 text-lg font-semibold">Active sharing links</h3>{invitations.filter(({ status }) => status === "pending").length ? <ul className="mt-3 space-y-3">{invitations.filter(({ status }) => status === "pending").map((invitation) => <li key={invitation.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/10 p-4"><div><p>{invitation.maxUses === null ? `${invitation.useCount} uses · unlimited` : `${invitation.useCount} of ${invitation.maxUses} uses`}</p><p className="text-sm text-[var(--muted)]">Expires {new Date(invitation.expiresAt).toLocaleDateString()}</p></div><RevokeInvitation slug={slug} invitation={invitation} /></li>)}</ul> : <p className="mt-3 text-sm text-[var(--muted)]">No active sharing links.</p>}<h3 className="mt-8 text-lg font-semibold">Pending membership requests</h3>{requests.length ? <ul className="mt-3 space-y-3">{requests.map((request) => <DecideRequest key={request.id} slug={slug} request={request} />)}</ul> : <p className="mt-3 text-sm text-[var(--muted)]">No pending membership requests.</p>}</section>;
}
