...
export default async function CommunitySettingsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const profile = <section className="mt-8 rounded-3xl border border-border bg-surface p-8"><h2 className="mb-6 text-2xl font-semibold">Profile, policies, and lifecycle state</h2><p className="mb-6 text-sm capitalize text-text-muted">Status: {settings.lifecycleStatus}</p>{archived ? <p className="text-sm text-text-muted">Restore this community from the Lifecycle tab to edit its profile and policies.</p> : <CommunitySettingsForm settings={settings} />}</section>;
  ...
  const lifecycle = <section className="mt-8 rounded-3xl border border-danger/30 bg-danger/10 p-8"><h2 className="text-2xl font-semibold">{archived ? "Restore community" : "Archive community"}</h2><p className="mt-3 text-text-muted">{archived ? "Restoring makes this community active again. Its memberships and records are preserved." : "Archiving hides this community and prevents normal use. It does not permanently delete any data."}</p><CommunityLifecycleForm slug={settings.slug} action={archived ? "restore" : "archive"} /></section>;

  return <main className="mx-auto min-h-screen max-w-4xl px-6 py-16">{!archived ? <Link href={`/communities/${settings.slug}`} className="text-sm text-brand hover:underline">← {settings.name}</Link> : null}<h1 className={archived ? "text-4xl font-semibold" : "mt-8 text-4xl font-semibold"}>Community administration</h1>{archived ? <p className="mt-4 rounded-xl bg-warning/10 p-4 text-warning">This community is archived. Its administration data is read-only until an owner restores it.</p> : null}<AdministrationTabs tabs={[{ id: "profile", label: "Profile", content: profile }, { id: "people", label: "People", badge: pendingRequests.length + pendingGmRequests.length, content: peopleTab }, { id: "sessions", label: "Sessions", content: <SessionsOverview slug={settings.slug} sessions={futureSessions} readOnly={archived} /> }, { id: "lifecycle", label: "Lifecycle", content: lifecycle }]} /></main>;
}
