...
type GmGrant = { id: string; displayName: string; status: string; futureSessions?: { id: string; label: string }[] };
...
function Feedback({ state }: { state: OwnerGmActionState }) {
  return <>{state.error ? <p role="alert" className="mt-2 text-sm text-danger">{state.error}</p> : null}{state.success ? <p role="status" className="mt-2 text-sm text-success">{state.success}</p> : null}</>;
}
...
export function GmManagement({ slug, requests, grants }: { slug: string; requests: GmRequest[]; grants: GmGrant[] }) {
  return <section className="mt-8 rounded-3xl border border-border bg-surface p-8"><h2 className="text-2xl font-semibold">Game Masters</h2><h3 className="mt-6 text-lg font-semibold">Pending GM requests</h3>{requests.length ? <ul className="mt-3 space-y-3">{requests.map((request) => <li key={request.id} className="rounded-xl border border-border p-4"><p className="font-semibold">{request.displayName}</p><p className="mt-1 text-sm text-text-muted">Requested {new Date(request.requestedAt).toLocaleDateString()}</p><div className="mt-4 flex flex-wrap gap-3"><Decision slug={slug} request={request} decision="approve" /><Decision slug={slug} request={request} decision="reject" /></div></li>)}</ul> : <p className="mt-3 text-sm text-text-muted">No pending GM requests.</p>}<h3 className="mt-8 text-lg font-semibold">GM grant history</h3>{grants.length ? <ul className="mt-3 space-y-3">{grants.map((grant) => <li key={grant.id} className="rounded-xl border border-border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">{grant.displayName}</p><p className="text-sm capitalize text-text-muted">{grant.status}</p>{grant.futureSessions?.length ? <div className="mt-3 rounded-lg bg-warning/10 p-3 text-sm text-warning"><p className="font-semibold">Future assignments must be resolved before revocation:</p><ul className="mt-1 list-disc pl-5">{grant.futureSessions.map((session) => <li key={session.id}><a href={`/communities/${encodeURIComponent(slug)}/sessions/${session.id}`} className="hover:underline">{session.label}</a></li>)}</ul></div> : null}</div>{grant.status === "active" ? <RevokeGrant slug={slug} grant={grant} /> : null}</div></li>)}</ul> : <p className="mt-3 text-sm text-text-muted">No GM grants.</p>}</section>;
}
