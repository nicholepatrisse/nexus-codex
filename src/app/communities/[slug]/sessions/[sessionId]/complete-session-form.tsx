  ...
  return <section className="mt-8 rounded-2xl border border-brand/30 bg-surface-raised p-5" aria-labelledby="completion-heading">
    ...
    <form action={formAction} className="mt-5 space-y-4" onSubmit={(event) => { if (!completed && !window.confirm(future ? "This session is in the future. Mark it complete anyway?" : "Mark this session complete?")) event.preventDefault(); }}>
      {characters.length > 1 ? <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border bg-surface p-4"><button type="button" className="rounded-full border border-border-strong px-4 py-2 text-sm font-semibold hover:border-brand hover:text-brand" onClick={(event) => {
        ...
        for (const field of ["advancementSpeed", "xp", "baseCreditsMinor", "partnerCode", "eventName", "eventCode", "gmOrganizedPlayId", "note"] as const) {
        ...
        setCopied(true);
      }}>Apply first Chronicle to all</button><span className="text-sm text-text-muted">Copies advancement, adventure rewards, shared metadata, and GM notes. Each character’s level and Downtime choices stay unchanged.</span>{copied ? <span role="status" className="text-sm font-semibold text-success">Applied.</span> : null}</div> : null}
      {characters.length ? characters.map((character) => { const preview = previews[character.characterId] ?? previewFor(character); const downtimeDays = preview.xp * 2; let earned: ReturnType<typeof calculateEarnIncome> | null = null; if (preview.downtimeDisposition === "earn_income" && preview.downtimeCheckTotal != null && preview.downtimeProficiency) { try { earned = calculateEarnIncome(character.level ?? 1, preview.downtimeCheckTotal, preview.downtimeProficiency, downtimeDays); } catch { earned = null; } } const downtimeCredits = preview.downtimeOverrideCreditsMinor ?? earned?.calculatedCreditsMinor ?? 0; const totalCredits = preview.baseCreditsMinor + downtimeCredits; return <div key={character.characterId} className="rounded-xl border border-border bg-surface p-4">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="text-sm font-semibold">Downtime disposition<select name={`downtimeDisposition:${character.characterId}`} defaultValue={character.downtimeDisposition} onChange={(event) => updatePreview(character.characterId, "downtimeDisposition", event.target.value as RewardPreview["downtimeDisposition"])} className="mt-1 block w-full rounded-lg border border-border bg-surface px-3 py-2 font-normal"><option value="earn_income">Earn Income</option><option value="other">Another activity</option><option value="declined">Declined / lost</option></select></label>
          ...
          <label className="text-sm font-semibold">Earn Income proficiency<select name={`downtimeProficiency:${character.characterId}`} defaultValue={character.downtimeProficiency ?? ""} onChange={(event) => updatePreview(character.characterId, "downtimeProficiency", (event.target.value || null) as DowntimeProficiency | null)} className="mt-1 block w-full rounded-lg border border-border bg-surface px-3 py-2 font-normal"><option value="">—</option><option value="trained">Trained</option><option value="expert">Expert</option><option value="master">Master</option></select></label>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2"><div><p className="text-sm font-semibold">Chronicle number</p><p className="mt-1 rounded-lg border border-border bg-surface-raised px-3 py-2 text-sm text-text-muted">{character.chronicleNumber || "Generated when applied"}</p></div>{[["downtimeCorrectionNote","Override correction note",character.downtimeCorrectionNote],["downtimeActivity","Other activity",character.downtimeActivity],["partnerCode","Partner Code",character.partnerCode],["eventName","Event name",character.eventName],["eventCode","Event number / code",character.eventCode],["gmOrganizedPlayId","GM Organized Play ID",character.gmOrganizedPlayId]].map(([field,label,value]) => <label key={field} className="text-sm font-semibold">{label}<input name={`${field}:${character.characterId}`} required={field === "eventName" || field === "eventCode"} defaultValue={value} className="mt-1 block w-full rounded-lg border border-border bg-surface px-3 py-2 font-normal" /></label>)}</div>
