...
export default async function SessionPage({ params }: { params: Promise<{ slug: string; sessionId: string }> }) {
  ...
  if (!session || (session.status !== "draft" && session.status !== "published")) notFound();
  ...
  if (session.status === "draft" ? !isOwner && !isAssignedGm : !canViewSchedule) notFound();
