...
import { characters, chronicles, contentItems, people, sessionGmCredits, sessionSignups, sessions } from "@/db/schema";
...
export default async function SessionPage({ params, searchParams }: { params: Promise<{ slug: string; sessionId: string }>; searchParams?: Promise<{ completed?: string }> }) {
  ...
  if (session.status !== "draft") {
    ...
    if (isManager) completionCharacters = rows.flatMap((row) => row.status === "confirmed" && row.characterId && row.characterName ? [{ characterId: row.characterId, characterName: row.characterName, playerName: row.personName, societyNumber: row.characterSocietyNumber, level: progressionByCharacter.get(row.characterId)?.currentLevel, className: row.characterClassName, relationship: "Player" as const, gmNotes: "", advancementSpeed: "standard" as const, xp: 0, creditsMinor: 0, reputation: 0, downtime: 0 }] : []);
