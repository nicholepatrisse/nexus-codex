...
import { formatCharacterIdentityText } from "@/character/character-identity";
...
function Assignment({ slug, sessionId, participant }: { slug: string; sessionId: string; participant: UnassignedParticipant }) {
  ...
  return <form action={action} className="min-w-0 rounded-xl border border-warning/30 bg-warning/10 p-4"><p className="font-semibold">{participant.personName}</p>{participant.characters.length ? <div className="mt-3 grid min-w-0 items-end gap-3 sm:grid-cols-[minmax(0,1fr)_auto]"><label className="min-w-0 text-sm font-semibold">Character<select name="characterId" required defaultValue="" className="mt-1 block w-full min-w-0 rounded-lg border border-border-strong bg-surface px-3 py-2 font-normal"><option value="" disabled>Choose a character</option>{participant.characters.map((character) => <option key={character.id} value={character.id}>{formatCharacterIdentityText({ ...character, level: character.currentLevel })}</option>)}</select></label><button type="submit" disabled={pending} className="w-fit rounded-full bg-brand px-4 py-2 text-sm font-semibold whitespace-nowrap text-on-brand disabled:opacity-60">{pending ? "Assigning…" : "Assign character"}</button></div> : <p className="mt-2 text-sm text-text-muted">This player has no eligible characters.</p>}{state.error ? <p role="alert" className="mt-2 text-sm text-danger">{state.error}</p> : null}</form>;
}
