...
export function CancelSessionButton({ slug, sessionId }: { slug: string; sessionId: string }) {
  ...
  return <form action={action} onSubmit={(event) => { if (!window.confirm("Cancel this session? Players will no longer be able to sign up.")) event.preventDefault(); }}><button type="submit" disabled={pending} className="rounded-full border border-red-300/30 px-5 py-2.5 text-sm font-semibold text-red-200 disabled:opacity-60">{pending ? "Cancelling…" : "Cancel session"}</button>{state.error ? <p role="alert" className="mt-2 text-sm text-red-300">{state.error}</p> : null}</form>;
}
