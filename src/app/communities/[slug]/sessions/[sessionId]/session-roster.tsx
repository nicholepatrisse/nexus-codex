...
export function SessionRoster({ capacity, confirmedCount, waitlistedCount, entries }: { capacity: number; confirmedCount: number; waitlistedCount: number; entries?: SessionRosterEntry[] }) {
  ...
  return <section className="mt-8 border-t border-border pt-6"><h2 className="text-xl font-semibold">Players</h2><p className="mt-2 text-sm text-text-muted">{confirmedCount} of {capacity} seats confirmed · {waitlistedCount} waitlisted</p>{entries ? <div className="mt-5 grid gap-6 sm:grid-cols-2"><div><h3 className="text-sm font-semibold text-text-muted">Confirmed</h3>{confirmed.length ? <ol className="mt-2 space-y-2">{confirmed.map((entry) => <li key={entry.id} className="rounded-lg border border-border px-3 py-2 text-sm"><Player entry={entry} /></li>)}</ol> : <p className="mt-2 text-sm text-text-muted">No confirmed players.</p>}</div><div><h3 className="text-sm font-semibold text-text-muted">Waitlist</h3>{waitlisted.length ? <ol className="mt-2 space-y-2">{waitlisted.map((entry) => <li key={entry.id} className="rounded-lg border border-border px-3 py-2 text-sm"><span className="mr-2 text-text-muted">#{entry.waitlistPosition}</span><Player entry={entry} /></li>)}</ol> : <p className="mt-2 text-sm text-text-muted">No one is waiting.</p>}</div></div> : null}</section>;
}
