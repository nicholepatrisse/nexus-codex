...
export default async function NewSessionDraftPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const eligibleCommunities = await listGameCreationCommunitiesForPerson(actor.personId);
  if (!eligibleCommunities.some((community) => community.id === access.community.id)) notFound();
