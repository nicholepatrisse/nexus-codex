...
import { ScenarioSelector } from "@/app/scenario-selector";
...
export function SessionDraftForm({
  ...
  return <form action={formAction} className="mt-10 space-y-6" noValidate onSubmit={(event) => {
    ...
    <ScenarioSelector groups={scenarioGroups.map((group) => ({ label: group.label, options: group.options.map((option) => { const [code, ...title] = option.label.split(": "); return { id: option.id, code: code ?? option.label, title: title.join(": ") }; }) }))} value={contentItemId} invalid={Boolean(state.fieldErrors?.contentItemId)} onValueChange={setContentItemId} previewScenario={previewScenario} addScenario={addScenario} />
    ...
    <fieldset><legend className="block text-sm font-semibold">Style</legend><div className="mt-2 grid gap-3 sm:grid-cols-2"><SelectionCard name="locationType" value="virtual" title="Virtual" description="Played online" checked={locationType === "virtual"} onChange={() => setLocationType("virtual")} /><SelectionCard name="locationType" value="physical" title="Physical" description="Played in person" checked={locationType === "physical"} onChange={() => setLocationType("physical")} /></div></fieldset>
