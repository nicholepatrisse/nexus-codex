...
import { characterInventoryEntries, characterOptionSelections, characters } from "@/db/schema";
...
export async function getCharacterValidationReview(actor: AuthenticatedActor, characterId: string, database: Database = getDb()) {
  ...
  const [context, entries, optionSelections] = await Promise.all([
    ...
    database.select().from(characterOptionSelections).where(eq(characterOptionSelections.characterId, characterId)).orderBy(asc(characterOptionSelections.acquiredLevel), asc(characterOptionSelections.createdAt)),
  ]);
  ...
  return { summary: deriveCharacterValidationSummary(character, context, inventory, optionSelections), isOwner: character.isOwner };
}
