...
export type CharacterValidationDetail = Readonly<{ key: string; category: "Class" | "Ancestry" | "Background" | "Heritage" | "Feat" | "Inventory"; selection: string; source: string | null; sourceHref: string | null; materialHref?: string | null; playerNote: string | null; sourceChronicleHref?: string | null; editHref: string; status: Exclude<ValidationStatus, "validated">; issues: readonly ValidationIssue[] }>;
...
const materialHref = (issues: readonly ValidationIssue[], identity?: string | null) => issues.some(({ type }) => type === "missing_material_ownership") && identity ? `/profile?tab=materials&material=${encodeURIComponent(identity)}` : null;
...
export function deriveCharacterValidationSummary(character: CharacterDetail, context: IdentityValidationContext, inventory: readonly ValidatedInventoryEntry[], characterOptions: readonly CharacterOptionSelection[] = []): CharacterValidationSummary {
  ...
  for (const field of identityFields) {
    ...
    selections.push({ status: validation.status, detail: validation.status === "validated" ? undefined : { key: `identity-${field.type}`, category: field.category, selection: value, source: option?.sourceMaterialTitle ?? null, sourceHref: option?.sourceUrl ?? null, materialHref: materialHref(validation.issues, option?.sourceMaterialIdentity), playerNote: character[field.note], editHref: `/characters/${character.id}/edit#${field.anchor}`, status: validation.status, issues: validation.issues } });
  }
  for (const selection of characterOptions) {
    const validation = validateCharacterOptionSelection(selection, character, context);
    selections.push({ status: validation.status, detail: validation.status === "validated" ? undefined : { key: `option-${selection.id}`, category: selection.selectionKind === "heritage" ? "Heritage" : "Feat", selection: selection.nameSnapshot, source: selection.sourceMaterialTitleSnapshot, sourceHref: selection.sourceUrlSnapshot, materialHref: materialHref(validation.issues, selection.sourceMaterialIdentitySnapshot), playerNote: selection.validationNote, sourceChronicleHref: selection.sourceChronicleId ? `/characters/${character.id}/chronicles/${selection.sourceChronicleId}` : null, editHref: `/characters/${character.id}?tab=feats#option-${encodeURIComponent(selection.id)}`, status: validation.status, issues: validation.issues } });
  }
  for (const entry of inventory) selections.push({ status: entry.validation.status, detail: entry.validation.status === "validated" ? undefined : { key: `inventory-${entry.id}`, category: "Inventory", selection: entry.itemNameSnapshot, source: entry.sourceMaterialTitle, sourceHref: entry.itemLinkSnapshot, materialHref: materialHref(entry.validation.issues, entry.sourceMaterialIdentity), playerNote: entry.validationNote, editHref: `/characters/${character.id}/inventory/${entry.id}/edit`, status: entry.validation.status, issues: entry.validation.issues } });
