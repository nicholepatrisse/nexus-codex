...
export function IdentityOptionField({ type, value, onValueChange, note, onNoteChange, context, hasChronicleAccess = false, invalid = false }: { type: Type; value: string; onValueChange: (value: string) => void; note: string; onNoteChange: (value: string) => void; context: IdentityValidationContext; hasChronicleAccess?: boolean; invalid?: boolean }) {
  ...
  return <AdvisorySelectionField type={type} value={value} context={validationContext} note={note} onNoteChange={onNoteChange} hasChronicleAccess={hasChronicleAccess} onMaterialAdded={(identities) => setAddedMaterialIdentities((current) => [...new Set([...current, ...identities])])}>
