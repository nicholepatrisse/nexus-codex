...
export const characters = pgTable(
  "characters",
  {
    ...
    importAdapterVersion: integer("import_adapter_version"),
  ...
  (table) => [
    ...
    uniqueIndex("characters_person_creation_idempotency_unique").on(table.personId, table.creationIdempotencyKey).where(sql`${table.creationIdempotencyKey} is not null`),
    ...
    check("characters_import_adapter_version_check", sql`${table.importAdapterVersion} is null or ${table.importAdapterVersion} > 0`),
    check("characters_import_digest_check", sql`${table.importDigest} is null or length(${table.importDigest}) = 64`),
...
export const characterOptionSelections = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("character_option_selections_import_key_unique").on(table.characterId, table.importSource, table.importKey).where(sql`${table.importSource} is not null`),
    ...
    check("character_option_selections_import_source_check", sql`${table.importSource} is null or ${table.importSource} = 'pathbuilder-pathmuncher'`),
    check("character_option_selections_import_key_check", sql`${table.importKey} is null or length(btrim(${table.importKey})) between 1 and 300`),
  ],
);
