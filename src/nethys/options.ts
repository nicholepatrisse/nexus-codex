...
import { normalizeMaterialIdentity } from "@/materials/materials";
...
export function parseNethysOptionHtml(html: string, sourceUrl: string): NethysOption {
  ...
  const sourceMaterialTitle = $(".sources").first().text().replace(/^\s*Source\s*/i, "").replace(/\s+/g, " ").trim() || undefined;
  const traits = $(".trait, .traits a, a.link-trait").map((_i, element) => $(element).text().trim()).get().filter(Boolean);
  return { name, optionType, sourceMaterialTitle, sourceMaterialIdentity: sourceMaterialTitle ? normalizeMaterialIdentity(sourceMaterialTitle) : undefined, sourceUrl: url.href, metadata: { traits: [...new Set(traits)] } };
}
...
export async function importNethysOption(value: string, database = getDb(), fetcher: typeof fetch = fetch) {
