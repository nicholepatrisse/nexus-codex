...
describeWithDatabase("characters persistence", () => {
  ...
  it("creates and lists characters only for the authenticated actor", async () => {
    ...
    await getDb().insert(gameSystems).values({ id: gameSystemId, code: gameSystemId, name: "Character Test" });
    ...
    expect((await getDb().select().from(characters))[0]?.personId).toBe(owner.person.id);
  });
});
