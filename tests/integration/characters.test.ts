...
describeWithDatabase("characters persistence", () => {
  ...
  it("owner-manages manual Chronicles without changing identity", async () => {
    ...
    expect(await getCharacterDetail(ownerActor, character.id)).toEqual(expect.objectContaining({ xp: 0, creditsMinor: 150, reputation: 0, downtime: 0 }));
    ...
    expect(await getCharacterDetail(ownerActor, character.id)).toEqual(expect.objectContaining({ xp: 12, currentLevel: 2, creditsMinor: 375, reputation: 3, downtime: 9 }));
  ...
  it("owner-manages distinct inventory lots without ledger side effects", async () => {
    ...
    const source = await createManualChronicle(ownerActor, character.id, { scenarioNumber: "1-01", scenarioName: "Source", datePlayed: "2026-08-20", characterLevel: 1, advancementSpeed: "standard", xp: 0, creditsMinor: 0, reputation: 0, downtime: 0 });
    const wrongSource = await createManualChronicle(otherActor, otherCharacter.id, { scenarioNumber: "1-02", scenarioName: "Wrong source", datePlayed: "2026-08-20", characterLevel: 1, advancementSpeed: "standard", xp: 0, creditsMinor: 0, reputation: 0, downtime: 0 });
