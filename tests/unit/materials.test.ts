...
import { addKnownOwnedMaterial, addReferencedOwnedMaterial, materialMatchesReference, materialValidationIdentities, normalizeMaterialIdentity, parseNethysProductUrl, parsePaizoMaterialHtml, PLAYER_CORE } from "@/materials/materials";
...
describe("materials", () => {
  ...
  it("uses a normalized ISBN-13 as the canonical material identity", () => {
    ...
    expect(result).toMatchObject({ isbn: "9781640787124", identity: "isbn-9781640787124", productCode: "PZO24006" });
  });
  ...
  it("adds a known database material without another external lookup", async () => {
    let inserted = false;
    const known = { id: "existing", personId: "other-player", identity: "isbn-9781640787124", isbn: "9781640787124", productCode: "PZO24006", title: "Starfinder Adventure: Guilt of the Grave World", sourceUrl: "https://store.paizo.com/starfinder-adventure-guilt-of-the-grave-world/", aliases: ["Guilt of the Grave World"], createdAt: new Date(), updatedAt: new Date() };
    const database = { select: () => ({ from: async () => [known] }), insert: () => ({ values: () => { inserted = true; return { onConflictDoUpdate: async () => undefined }; } }) };
